...
interface TransactionDao {
    ...
    @Query("SELECT COALESCE(SUM(amount), 0) FROM transactions WHERE type = 'EXPENSE' AND date LIKE :yearMonth || '%'")
    fun observeMonthlyExpense(yearMonth: String): Flow<Double>

    @Query("SELECT category, COALESCE(SUM(amount), 0) as total FROM transactions WHERE type = 'EXPENSE' GROUP BY category ORDER BY total DESC")
